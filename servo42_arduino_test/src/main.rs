...
fn TIMER0_COMPA() {
    avr_device::interrupt::free(|cs| {
        ...
        let counter = counter_cell.get()+MILLIS_INCREMENT;
        if counter%1024==4{
            counter_cell.set(counter+4);
        }else{
...
fn main() -> ! {
    ...
    loop {
        ...
        //let time = millis();
        ufmt::uwriteln!(&mut serial, "{}", millis()).void_unwrap();
        let x = millis()+1000;
        while millis()<x {}
