use std::{time::{Duration, SystemTime}, thread::{sleep, self, spawn}, sync::mpsc::{self, channel}};

use eframe::egui::{self, Button, TextEdit};
use motor::servo42::Servo42C;
use serial::{standard::{serialport, serialport::*}, test::SerialTest};
fn main() {
...
impl MyEguiApp {
    fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        //m.calibrate().unwrap();*/
        MyEguiApp{ m: Default::default(), name: "/dev/ttyACM0".to_string(), is_connect: false}
       ...
       egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical(|ui|{
                ui.horizontal(|ui|{
                    ...
                    if ui.add(egui::Checkbox::new(&mut self.is_connect, "Connect")).changed() {
                        if self.is_connect{
                            println!("connecting");
                            if let Ok(val) = serialport::new("/dev/ttyACM0", 115_200)
                                ...
                                let m = Servo42C::<Box<dyn SerialPort>>{
                                    address: 0xe0, s: val, kp: 0x650, ki: 0x120, kd: 0x650, acc: 0x11e };
                                self.m=Some(m);
                            }else{
